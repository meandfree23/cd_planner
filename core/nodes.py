...
def idea_node(state: PlannerState) -> PlannerState:
    ...
    prompt = ChatPromptTemplate.from_messages([
        ("system", """당신은 세계적인 크리에이티브 디렉터입니다. 앞서 도출된 '팩트 기반의 분석(Reason Why)'을 바탕으로, 템플릿의 한계를 깨부수는 창의적 아이디어와 '마이크로 모먼츠' 전략을 도출하세요.
...
4. **🔥 [가장 중요한 포인트] 한계 돌파 전략의 당위성 (Why We Broke the Boundaries)**:
   단순히 기이하거나 파격적인 기믹(Stunt)을 제시하는 데 그치지 마세요. **"왜 기존 관성의 마케팅을 버리고, 왜 하필 이 파격적인 은유/제약 조건으로 판을 흔들어야만 하는가?"**에 대한 냉철하고 치명적인 경영학적/심리학적 당위성(Reason Why)을 반드시 명확히 입증하세요.
...
1. 메인 기획의 뼈대가 부실해지지 않도록, 이 파트에서만 **반드시 10장~12장 분량**으로 슬라이드를 꽉 채워 작성하세요. 다음 핵심 내용들이 충분히 세분화되어 슬라이드(## Slide X)로 촘촘하게 전개되어야 합니다:
   ...
   - [🔥 한계 돌파 전략의 당위성 해부 (Why We Broke the Boundaries)]: 기존 카테고리 관성(Conventional Marketing Myopia)을 버리고, 왜 하필 이 파격적인 은유/제약 조건으로 판을 흔들어야만 하는지 명확하고 치명적인 경영학적/심리학적 명분(Reason Why) 제시
...
4. [필수 포함 항목] 각 슬라이드의 내용 중 반드시 다음 두 가지가 명시되어야 합니다:
   - **[전략적 도입 이유 (Reason Why)]**: 이 레퍼런스가 해당 기업의 본질적 자산 및 기획안과 왜 완벽히 부합하는지에 대한 논리적 설득.
   - **[레퍼런스 직접 확인 링크]**: 슬라이드 하단에 시각적으로 돋보이게 `[🔍 레퍼런스 구글 검색하기](https://www.google.com/search?q=정확한+캠페인+이름)` 형식의 클릭 가능한 링크를 배치하세요.
5. 각 슬라이드의 [PT 스크립트]는 통계적 수치와 예술적 미학을 바탕으로 한, 가장 밀도 높고 통찰력 있는 장문의 논술(최소 300자 이상)이어야 합니다.
6. 미래 발전 마일스톤이나 예술적 상호작용 프로세스를 보여주는 `mermaid` 차트를 최소 2개 삽입하세요.
...
def evaluation_node(state: PlannerState) -> PlannerState:
    print("--- [NODE] MULTI-JURY EVALUATION (슬라이드별 다면 심사 위원회) ---")
    llm = get_openai_llm()
    brief = state["brief"]
    goal = state.get("goal", "")
    
    prompt = ChatPromptTemplate.from_messages([
        ("system", """당신은 세계 최고 권위의 '칸 라이언즈(Cannes Lions) 급 슬라이드별 다면 심사 위원회'입니다.
프로젝트 목표(Goal)에 따라 4인의 핵심 심사위원 가중치를 자동 조정하여 30개 슬라이드 전체를 슬라이드 단위(Slide-by-Slide)로 정밀 검수하세요.

[심사위원 4인 핵심 페르소나]
1. 🎨 칸 광고제 크리에이티브 심사위원장 (Art/Creative): 메타포의 파괴력, 슬라이드별 훅/카피 미학 검증.
2. 📈 글로벌 퍼포먼스 마케팅 디렉터 (ROI/Data): 리서치 팩트(80%), 수치 인과관계, KPI 타당성 검증.
3. 💼 CFO & 비즈니스 전략 파트너 (ROI/Scale): 경영학적 당위성(Why We Broke Boundaries) 및 비대칭 우위 검증.
4. 🔥 Z세대 밈 & 트렌드 해커 (Era/Trend): 감각적 문법, 독소 요소 배제, 실질적 전파력 검증.
...
- **한 줄 심사평**: [네 심사위원의 의견을 종합한 촌철살인 한 줄 평]

## 2. 👩‍⚖️ 심사위원별 핵심 피드백
- **🎨 크리에이티브 심사위원장**: [지적 및 핵심 평가]
- **📈 퍼포먼스 마케팅 디렉터**: [지적 및 핵심 평가]
- **💼 CFO & 비즈니스 파트너**: [지적 및 핵심 평가]
- **🔥 Z세대 트렌드 해커**: [지적 및 핵심 평가]

## 3. 🎯 슬라이드별 원핀(Surgical) 미흡점 핀포인팅 (10점 만점 갭 수술 지점)
*전체 슬라이드 중 가장 미흡한 2~4개 슬라이드를 정확히 짚어내어 10점 만점으로 올리기 위한 원핀 지침을 명시하세요:*
- **[Slide X: 슬라이드 제목]**: ⚠️ 미흡점: [원인] ➔ 🔧 10점 만점 수술 지시: [구체적 수정안]
- **[Slide Y: 슬라이드 제목]**: ⚠️ 미흡점: [원인] ➔ 🔧 10점 만점 수술 지시: [구체적 수정안]

## 4. 🛠️ 10점 만점 집도를 위한 최종 수정 명령 (Surgical Pinpoint Requests)
1. [슬라이드 X 원핀 보완 지시]
2. [슬라이드 Y 원핀 보완 지시]
3. [전체 논리/대본 밀도 10점 만점 고도화 지시]
...
def evolution_proof_node(state: PlannerState) -> PlannerState:
    print("--- [NODE] EVOLUTION PROOF (원핀 슬라이드 진화 증명 에이전트) ---")
    llm = get_openai_llm()
    
    revision_count = state.get("revision_count", 0)
    # 만약 재작업(루프)이 한 번도 발생하지 않고 초안이 곧 최종안(점수 9점 이상 통과)이라면, 진화 증명 생략
    if revision_count <= 1:
        return {"evolution_proof": "✨ **[초안 통과 (10점 만점)]** 다면 심사 위원회의 엄격한 기준(9.0점 이상)을 단번에 통과하여, 별도의 슬라이드 수술(진화) 없이 완벽히 확정된 마스터피스입니다."}
        
    prompt = ChatPromptTemplate.from_messages([
        ("system", """당신은 시스템의 투명성을 담당하는 '원핀 슬라이드 진화 증명(Evolution Proof) 에이전트'입니다.
AI가 '심사위원회의 슬라이드별 원핀 지적'을 수용하여 '초안(V1)'에서 '최종 10점 만점 마스터안'으로 진화할 때, 슬라이드 단위로 실제로 무엇이 어떻게 수술되어 10점 만점으로 격상되었는지(Delta)를 명백하게 증명하세요.

[출력 양식 (마크다운)]
# 🧬 원핀(Surgical) 슬라이드 10점 만점 진화 증명 보고서

이 기획서는 다면 심사 위원회의 뼈아픈 슬라이드별 지적을 원핀 수술하여 다음과 같이 10점 만점으로 진화했습니다.

## 1. 🎯 슬라이드별 원핀(Surgical) 수술 및 10점 만점 진화 (Delta)
- **[Slide X: 슬라이드 제목]**
  - **⚠️ 수술 전 미흡점**: [심사위원회 지적 내용]
  - **🔥 10점 만점 수술 결과**: [최종안의 해당 슬라이드에 반영된 팩트/논리/대본의 명확한 개선점 증명]
- **[Slide Y: 슬라이드 제목]**
  - **⚠️ 수술 전 미흡점**: [심사위원회 지적 내용]
  - **🔥 10점 만점 수술 결과**: [최종안의 해당 슬라이드에 반영된 팩트/논리/대본의 명확한 개선점 증명]

## 2. 💡 10점 만점 달성 평가 총평
- [슬라이드별 핀포인트 수술을 통해 기획서의 방어율과 퀄리티가 10점 만점 수준으로 올라간 소회 요약]"""),
        ("user", "다면 심사 위원회 지적사항:\n{feedback}\n\n초안(V1) 원문:\n{v1_report}\n\n최종안 원문:\n{final_report}")
    ])
...
4. [핵심 필수 요소] 각 레퍼런스 제안 시 다음 2가지를 반드시 명시하세요:
   - **전략적 도입 이유 (Reason Why)**: 이 레퍼런스가 왜 타겟의 텐션을 해결하고 브랜드 자산을 극대화하는지 구체적인 인과관계.
   - **직접 확인 링크 (Verification Link)**: 사용자가 해당 레퍼런스를 바로 눈으로 확인할 수 있도록, 정확한 키워드가 포함된 검색 URL을 1개 이상 반드시 제공하세요. (예: `[🔍 구글 검색하기](https://www.google.com/search?q=브랜드명+캠페인명)` 또는 `[▶️ 유튜브 검색하기](https://www.youtube.com/results?search_query=브랜드명+캠페인명)`)
**[중요] 반드시 모든 문장과 단어를 한국어(Korean)로 상세하게 작성하세요.**"""),
            ("user", "브리프: {brief}\n\n도출된 아이디어:\n{agile_ideas}\n\n마케팅 전략:\n{perf_data}")
        ])
